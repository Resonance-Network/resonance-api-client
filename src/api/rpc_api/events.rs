...
mod tests {
	...
	use ac_primitives::ResonanceRuntimeConfig;
	use codec::{Decode, Encode};
	...
	) -> Api<ResonanceRuntimeConfig, RpcClientMock> {
		// Create new api.
	...
	fn retrieve_extrinsic_index_from_block_works() {
		...
		let xt_hash1 = <ResonanceRuntimeConfig as Config>::Hasher::hash(&xt1);
		let xt_hash2 = <ResonanceRuntimeConfig as Config>::Hasher::hash(&xt2);
		let xt_hash3 = <ResonanceRuntimeConfig as Config>::Hasher::hash(&xt3);
